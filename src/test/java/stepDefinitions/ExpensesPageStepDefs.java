...
public class ExpensesPageStepDefs {
    ...
    @When("must be a numeric input field that only allows positive numbers")
    ...
    public void the_user_should_be_able_to_move_forward() {
    ...
    public void the_user_should_see_an_error_message_for_input() {
        SeleniumUtils.waitForVisibility(new ExpensesPage().getMonthlyRentalPaymentError(), 5);
        Assert.assertNotEquals( "Please enter a value greater than or equal to 1.", new ExpensesPage().getMonthlyRentalPaymentError().getText());
    }



    @When("must be a not numeric input field")
    public void must_be_a_not_numeric_input_field() throws InterruptedException {
        new ExpensesPage().clearmonthlyRentPayBox();
        Thread.sleep(5000);
